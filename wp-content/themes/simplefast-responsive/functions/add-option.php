...
function fastestwp_first_image() {
  global $post, $posts;
  $first_img = 'http://portable-power.pagodabox.com/wp-content/themes/simplefast-responsive/images/thumb.gif';
 // ob_start();
 // ob_end_clean();
  //$output = preg_match_all('&src="(.*?)"&', $post->post_content, $matches);
 // $first_i = array_unique($matches[1]);
 // $first_img = $first_i[0];
